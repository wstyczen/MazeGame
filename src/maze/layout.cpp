...
Layout Layout::FromString(const std::string &string_format, const char blocked,
                          const char unblocked) {
  std::vector<std::vector<char>> as_vec;
  std::string row;
  std::stringstream layout_string(string_format);

  while (std::getline(layout_string, row)) {
    std::replace(row.begin(), row.end(), blocked, kWall);
...
LayoutSize Layout::size() const { return size_; }

uint16_t Layout::middle_row() const { return size_.rows / kStep; }
uint16_t Layout::bottom_row() const { return size_.rows - 2; }
uint16_t Layout::middle_col() const { return size_.cols / kStep; }
