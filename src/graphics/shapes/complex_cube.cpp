...
ComplexCube::ComplexCube(const glm::vec3 &posi,
                      ...
                      DynamicSolidFigure( MakeCubeFigure(size_, posi, pos, vertex_color, inner_color)),
                      start_position(posi){}
...
void ComplexCube::Roll(const glm::vec2& turn_vec, GLfloat scale) {
    ...
    move_x = (cos(float(-pose_y + 135.0f) * 3.1415f / 180.0f) - cos(float(-prev_pose_y + 135) * 3.1415 / 180)) * scale*size_ / sqrt(2);
    move_y = -(cos(float(-pose_x + 135.0f) * 3.1415f / 180.0f) - cos(float(-prev_pose_x + 135) * 3.1415 / 180)) * scale*size_ /sqrt(2);
...
void ComplexCube::Act(){
    if(move_state == ComplexCube::FigureState::move_north){
      if( (pose_.x - floor(pose_.x / 90) * 90.0f) >= -ang_vel.x){
        ...
        ang_vel.x -= cos((pose_.x - floor(pose_.x / 90) * 90.0f) * 3.1415 / 180) * move_settings.acceleration;
    ...
    else if (move_state == ComplexCube::FigureState::move_east)
    {
      if( (pose_.y - floor(pose_.y / 90) * 90.0f) >= ang_vel.y){
        ...
        ang_vel.y += sin((pose_.y - floor(pose_.y / 90) * 90.0f) * 3.1415 / 180) * move_settings.acceleration;
      ...
      if((pose_.x - floor(pose_.x / 90) * 90.0f) >= ang_vel.x){
        Roll({ang_vel.x, 0.0f}, move_settings.distance);
        ang_vel.x += sin((pose_.x - floor(pose_.x / 90) * 90.0f) * 3.1415 / 180) * move_settings.acceleration;
    ...
    else if (move_state == ComplexCube::FigureState::move_west)
    {
      if( (pose_.y - floor(pose_.y / 90) * 90.0f) >= -ang_vel.y){
        Roll({0.0f, ang_vel.y}, move_settings.distance);
        ang_vel.y -= cos((pose_.y - floor(pose_.y / 90) * 90.0f) * 3.1415 / 180) * move_settings.acceleration;
...
void ComplexCube::DiscretizatePosition(){
  ...
  discrete_x = roundf((position_.x - start_position.x) / (move_settings.distance*size_)) * (move_settings.distance*size_);
  discrete_y = roundf((position_.y - start_position.y) / (move_settings.distance*size_)) * (move_settings.distance*size_);
  SetPosition({start_position.x + discrete_x, start_position.y + discrete_y, start_position.z});
}

DynamicSolidFigure ComplexCube::MakeCubeFigure(const GLfloat &side, const glm::vec3 &posi, const glm::vec3 &pos, const glm::vec3 &vertex_color, const glm::vec3 &inner_color){
