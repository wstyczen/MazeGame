...
void GameWindow::InitFigures(const maze::Layout &maze){
  ...
  glm::vec3 maze_start_position = {-maze_width, -maze_height, -maze_height*maze_settings_.maze_scale};
  glm::vec3 cube_start_position = {-maze_width +1, -maze_height +1, -maze_height*maze_settings_.maze_scale};
  maze_ = std::make_unique<MazeFigure>(MazeFigure{MazeFigure::Layout2VecOfWalls(&maze), maze_settings_.maze_height, maze_start_position});
  ...
  std::vector<GLfloat> floor_vertices = {
    -(floor_width+wall_thickness/2), -(floor_height+wall_thickness/2), maze_start_position.z - 0.5f,
  ...
  floor_ = std::make_unique<SolidFigure>(floor_vertices, floor_indices, glm::vec3{0.0f, 0.0f, 0.0f}, glm::vec3{0.0f, 0.0f, 0.0f});
}
