...
GameWindow::GameWindow(const maze::GeneratorType& generator_type,
                       const uint16_t& maze_size) {
  Init();
  ...
  glViewport((mode->width - mode->height) / 2, -mode->height / 15, mode->height,
             mode->height);
  shader_ = std::make_unique<Shader>(".//graphics//shaders//default.vert",
  ...
  const std::unique_ptr<maze::Layout> maze_layout =
      generator->Get({maze_size, maze_size});
  maze_ = std::make_unique<MazeFigure>(
      MazeFigure{MazeFigure::Layout2VecOfWalls(maze_layout.get()),
