...
class SolidFigure {
  public:
    ...
    * @param[in] indices array including indices that connects vertices into triangles
    ...
    * @param[in] pos pose of a figure (Euler angle), {0.0f, 0.0f, 0.0f} by default.

    */
    SolidFigure(GLfloat *vertices, unsigned int vert_size, GLuint *indices,
    ...
    * each vertex is described by 6 floats: x, y, z, colorR, colorG, colorB
    * @param[in] indices vector including indices that connects vertices into triangles
    * each triangle is created by connectring 3 vertices (3 indices needed)
    * @param[in] posi position of a figure, {0.0f, 0.0f, 0.0f} by default.
    * @param[in] pos pose of a figure (Euler angle), {0.0f, 0.0f, 0.0f} by default.
    */
    SolidFigure(const std::vector<GLfloat> &vertices, const std::vector<GLuint> &indices,
                const glm::vec3 &posi,
                const glm::vec3 &pos);
    ...
    * @param[in] shader_id ID of a Shader object which will be used to display figure
    * */
    ...
    */
    ...
    Especially needed when GLFWwindow isn't in shape of a square, because it's assumed to be.
    *
    * @param[in] proj_mat New projection matrix, the best way to get that matrix is using glm::perspective() method.
    */
  ...
  protected:
    ...
    */
    static void CheckDataFormat(const std::vector<GLfloat> &vertices, const std::vector<GLuint> &indices){};
};
