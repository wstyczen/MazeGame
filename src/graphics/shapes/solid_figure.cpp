...
SolidFigure::SolidFigure(GLfloat *vertices,unsigned int vert_size, GLuint *indices, unsigned int indi_size,
    ...
    mvp_.model = glm::rotate(mvp_.model, glm::radians(pose_.x), glm::vec3(1.0f, 0.0f, 0.0f));
...
SolidFigure::SolidFigure(const std::vector<GLfloat> &vertices, const std::vector<GLuint> &indices, const glm::vec3 &posi, const glm::vec3 &pos){
  ...
  vao_ = std::unique_ptr<VAO>(new VAO());
  vao_->Bind();
  vbo_ = std::unique_ptr<VBO>(new VBO(vertices_array, vertices.size()*sizeof(GLfloat)));
  ebo_ = std::unique_ptr<EBO>(new EBO(indices_array, indices.size()*sizeof(GLuint)));
  vao_->LinkAttrib(*vbo_, 0, 3, GL_FLOAT, 6 * sizeof(float), (void *)0);
  vao_->LinkAttrib(*vbo_, 1, 3, GL_FLOAT, 6 * sizeof(float), (void*)(3 * sizeof(float)));
  vao_->Unbind();
  vbo_->Unbind();
