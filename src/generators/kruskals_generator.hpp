...
class KruskalsGenerator : public Generator {
 public:
  ...
  std::unique_ptr<Layout> Get(const uint16_t& N, const uint16_t& M) override;

 private:
  KruskalsGenerator();

  void InitializeCellTrees(const uint16_t& N, const uint16_t& M);
  void InitializeEdges(const uint16_t& N, const uint16_t& M);
